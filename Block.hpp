...
template<size_t n>
Block<_alphabet_size, _character_caster>::Block(
    const std::array<std::string, n>& lines)
{
    _root = new_vertex();
    _finish_vertex = new_vertex();
    _vertexes.at(_finish_vertex)._is_finished = true;
...
void Block<_alphabet_size, _character_caster>::optimise_vertexes()
{
    ...
    std::vector<bool> was_in_vertex(_vertexes.size(), false);
    ...
    while(!q_vertexes.empty())
    {
        ...
        {
            if(!was_in_vertex.at(current_vertexes.getNext(i)))
            {
                new_vertexes.emplace_back(Vertex<_alphabet_size>());
                vertexes_dict[current_vertexes.getNext(i)] =
                    new_vertexes.size() - 1;
                was_in_vertex.at(current_vertexes.getNext(i)) = true;
                q_vertexes.push(current_vertexes.getNext(i));
            }
            new_vertexes.at(new_vertex).setLink(i,
                                                vertexes_dict[current_vertexes.getNext(
                                                    i)]);
        }

        auto eps_links = current_vertexes.get_eps_links();
        for(size_t link_number = 0;
            link_number < eps_links.size(); ++link_number)
        {
            if(!was_in_vertex.at(eps_links.at(link_number)))
            {
                new_vertexes.emplace_back(Vertex<_alphabet_size>());
                vertexes_dict[eps_links.at(link_number)] =
                    new_vertexes.size() - 1;
                was_in_vertex.at(eps_links.at(link_number)) = true;
                q_vertexes.push(eps_links.at(link_number));
            }
            new_vertexes.at(new_vertex).add_eps_link(
                vertexes_dict[eps_links.at(link_number)]);
        }
    }
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
bool
Block<_alphabet_size, _character_caster>::is_equal_sub_trees(size_t vertex1,
                                                             size_t vertex2)
{
    if(vertex1 == vertex2)
    {
    ...
    if((vertex1 == _root && vertex2 != _root) ||
       (vertex1 != _root && vertex2 == _root))
    {
        return false;
    }
    ...
    for(size_t i = 0; i < _alphabet_size; ++i)
    {
        if(_vertexes.at(vertex1).getNext(i) !=
           _vertexes.at(vertex2).getNext(i) &&
           !is_equal_sub_trees(_vertexes.at(vertex1).getNext(i),
                               _vertexes.at(vertex2).getNext(i)))
        {
            return false;
        }
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
unsigned long long
Block<_alphabet_size, _character_caster>::optimise_links_crowler(
    size_t position)
{
    unsigned long long current_hash = 1;
    for(size_t alphabet_index = 0;
        alphabet_index < _alphabet_size; ++alphabet_index)
    {
        if(_vertexes.at(position).getNext(alphabet_index) != _root)
        {
            const auto sub_tree_hash = optimise_links_crowler(
                _vertexes.at(position).getNext(alphabet_index));

            bool flag = false;
            for(auto& may_be_clone:_sub_tree_hashes[sub_tree_hash])
            {
                if(is_equal_sub_trees(may_be_clone,
                                      _vertexes.at(position).getNext(
                                          alphabet_index)))
                {
                    flag = true;
                    _vertexes.at(position).setLink(alphabet_index,
                                                   may_be_clone);
                    break;
                }
            }
            if(!flag)
            {
                _sub_tree_hashes[sub_tree_hash].push_back(
                    _vertexes.at(position).getNext(alphabet_index));
            }
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
void Block<_alphabet_size, _character_caster>::print()
{
    for(size_t vertex = 0; vertex < _vertexes.size(); ++vertex)
    {
        ...
        {
            std::cout << _vertexes.at(vertex).getNext(j) << ' ';
        }
        std::cout << '|';
        for(auto& eps_link:_vertexes.at(vertex).get_eps_links())
        {
            std::cout << eps_link << ' ';
        }
        std::cout << '\n';
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
size_t Block<_alphabet_size, _character_caster>::get_root() const
{
    return _root;
}

template<size_t _alphabet_size, size_t (* _character_caster)(char)>
size_t Block<_alphabet_size, _character_caster>::size() const
{
    return _vertexes.size();
}

template<size_t _alphabet_size, size_t (* _character_caster)(char)>
void Block<_alphabet_size, _character_caster>::concatinate_impl(
    const Block& block)
{
    _vertexes.reserve(size() + block.size());

    std::vector<bool> was_in_vertex(block.size(), false);
    ...
    while(!q_vertexes.empty())
    {
        ...
        for(size_t alphabet_index = 0;
            alphabet_index < _alphabet_size; ++alphabet_index)
        {
            if(!was_in_vertex.at(current_vertexes.getNext(alphabet_index)))
            {
                vertexes_dict[current_vertexes.getNext(
                    alphabet_index)] = new_vertex();
                was_in_vertex.at(
                    current_vertexes.getNext(alphabet_index)) = true;
                q_vertexes.push(current_vertexes.getNext(alphabet_index));
            }
            _vertexes.at(current_vertex).setLink(alphabet_index,
                                                 vertexes_dict[current_vertexes.getNext(
                                                     alphabet_index)]);
        }
        ...
        for(size_t eps_link = 0; eps_link < eps_links.size(); ++eps_link)
        {
            ...
            {
                _vertexes.at(current_vertex).add_eps_link(
                    vertexes_dict[eps_links.at(eps_link)]);
            }else
            {
    ...
    const std::vector<Block<_alphabet_size, _character_caster>>& blocks)
{
    if(!blocks.empty())
    {
        Block tmp_block(blocks[0]);

        for(size_t bloc_number = 1; bloc_number < blocks.size(); ++bloc_number)
        {
            tmp_block.concatinate_impl(blocks[bloc_number]);
        }
    ...
    const std::array<Block<_alphabet_size, _character_caster>, n>& blocks)
{
    if(!blocks.empty())
    {
        ...
        for(size_t block_number = 1;
            block_number < blocks.size(); ++block_number)
        {
            tmp_block.concatinate_impl(blocks[block_number]);
        }
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
Block<_alphabet_size, _character_caster>::Block(Block&& other,
                                                char type) noexcept
{
    Block tmp_block(other);
    tmp_block.add_closure(type);
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
void Block<_alphabet_size, _character_caster>::concatinate()
{
...
template<typename... args>
void
Block<_alphabet_size, _character_caster>::add_string(const std::string& line,
                                                     args... Args)
{
    add_string_impl(line);
    add_string(Args...);
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
void Block<_alphabet_size, _character_caster>::add_string_impl(
    const std::string& line)
{
    ...
    for(auto& symbol:line)
    {
        ...
        if(_vertexes.at(pos).getNext(symbol_id) == 0)
        {
            size_t next_vertex = new_vertex();
            _vertexes.at(pos).setLink(symbol_id, next_vertex);
            pos = next_vertex;
        }else
        {
            pos = _vertexes.at(pos).getNext(symbol_id);
        }
    }
...
template<size_t _alphabet_size, size_t (* _character_caster)(char)>
template<typename... args>
Block<_alphabet_size, _character_caster>::Block(const std::string& line1,
                                                args... Args)
{
    _root = new_vertex();
    _finish_vertex = new_vertex();
